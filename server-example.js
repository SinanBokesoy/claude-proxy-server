const express = require('express');
  ...
  async function findOrderInSheet(orderNumber) {
      ...
      try {
          ...
          // Try both 'Orders' sheet and main 'Sheet1' - adjust as needed
          let response;
          try {
              ...
              // If Orders sheet doesn't exist, try Sheet1
              response = await sheets.spreadsheets.values.get({
                  spreadsheetId: process.env.GOOGLE_SPREADSHEET_ID,
                  range: 'Sheet1!A:Z',
              });
  ...
  // Helper function to find user by serial number
  ...
  async function addTokensToUser(serialNumber, tokensToAdd) {
      ...
      if (!userInfo) {
          ...
          const columnLetter = String.fromCharCode(65 + userInfo.tokenColumnIndex);
          const range = `Sheet1!${columnLetter}${userInfo.rowIndex}`;
  ...
  app.post('/api/claude', authenticateRequest, async (req, res) => {
      ...
      try {
          ...
          console.log('Making request to Claude API using curl...');
